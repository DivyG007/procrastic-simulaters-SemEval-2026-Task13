...
13 deﬁnes this problem through three subtasks:
...
data, potentially missing structural and data-ﬂow
...
We reproduce the baseline from CodeBERT using the provided training pipeline.
\\
The baseline uses CodeBERT backbone (125M params), linear layer over [CLS] for classification and AdamW optimizer. \\
For Task C, due to computational constraints, training is performed on 90K samples (~10\% of the provided data).
\subsection{Classification Head Improvements}
To improve representational capacity beyond the linear classifier, we added: \\
...
\end{minipage}
\caption{Baseline results on the test set.}
...
\begin{itemize}
    ...
    \item \textbf{BiLSTM head}: The transformer’s token-level outputs are passed through a bidirectional LSTM, whose final forward and backward hidden states are concatenated and fed to the classifier.
\end{itemize}
...
We ran preliminary ablation studies on a smaller subset of the data (2,000 training samples and 500 each for validation and testing). This allowed us to quickly evaluate the contribution of different features, identify useful components, and discard ineffective ones.
...
The \textbf{pure backbone} variants (rows 1--2) use standard cross-entropy loss, weight decay 0.01, and a linear LR schedule with no warmup. \\
The \textbf{BiLSTM} variants (rows 3--4) use a cosine LR schedule with 10\% linear warmup and early stopping (patience\,=\,3); the BiLSTM head has hidden size 256 (bidirectional $\rightarrow$ 512-dim), a single dense block (LayerNorm + GELU), and dropout 0.15. \\
The \textbf{stylometric fusion} variants (rows 5--6) apply label smoothing ($\alpha\!=\!0.05$), weight decay 0.02, gradient accumulation over 2 steps (effective batch 32), and a differential learning rate scheme: backbone receives $2\!\times\!10^{-5}$ while the newly initialised head layers receive $2\!\times\!10^{-4}$ ($10\times$ multiplier). 
...
\paragraph{Dense Layers Degrade Performance.}
Deeper heads increase representational capacity, but they also introduce a larger number of trainable parameters. Simpler classification heads worked better across different data quantities (2k, 10k and 100k+).

\paragraph{BiLSTM Heads Underperform with Limited Data}
Both BiLSTM variants underperform simpler heads, likely because the extra layers provide redundant information already captured by the transformer, adding noise.

\paragraph{UniXcoder underperforms CodeBERT.}
UniXcoder's richer pre-training makes it more sensitive to fine-tuning conditions and dataset size; additionally it may not capture signal needed for this task.

% ─────────────────────────────────────────────────────────────────────────────
\subsubsection{Final Model}

Based on the ablation findings, our final Subtask~A model is \textbf{CodeBERT} fine-tuned with hyperparameters, early stopping, 8 stylometric features, and 10,000 training samples.
...
\bottomrule
\end{tabularx}
\caption{Subtask~A final model vs.\ baseline on the test set.}
\label{tab:taskA_final}
\end{table}

\subsubsection{Discussion}
The stylometric signals introduce lightweight indicators of coding style. These cues can sometimes differ between human-written and machine-generated code, providing a small additional signal for the classifier. Future improvements will be done on this base model.
...
\begin{enumerate}
    \item \textit{Validation loss divergence}: After reaching a minimum of 0.367 at epoch~3, the validation loss steadily increased to 0.449 by epoch~5.7 with a 22\% increase, clearly indicating clear overfitting. The model began memorizing training-set patterns rather than learning generalizable authorship features.
...
We have presented the approach of team Procastic Simulators to SemEval-2026 Task~13 on detecting machine-generated code. Our work till now addresses first subtasks A and B, with the primary focus to date on Subtask~B (11-class authorship attribution), where extreme class imbalance (88.4\% human) poses the primary challenge, and Subtask~A (binary detection), where we explore backbone and classification head alternatives.
